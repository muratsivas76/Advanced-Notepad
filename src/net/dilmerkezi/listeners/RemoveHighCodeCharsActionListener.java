...
/**
 * Listener responsible for removing characters with a Unicode value higher than a user-defined limit.
 * Useful for filtering out specific scripts like Arabic or special symbols.
 *
 * Licensed under GPL v3.
 */
public class RemoveHighCodeCharsActionListener implements ActionListener {
  
  private final JTextPane txt;
  private final JLabel statusLabel;
  private final Component parent;
  
  public RemoveHighCodeCharsActionListener(JTextPane txt, JLabel statusLabel, Component parent) {
    this.txt = txt;
          ...
          try {
            ...
            SwingUtilities.invokeLater(new Runnable() {
                ...
                public void run() {
                  ...
                  statusLabel.setText("Removed Chars Bigger Than: " + MIL);
                }
            });
  ...
  /**
   * Filters the text by character code values.
   */
  private String processText(String content, int limit) throws IOException {
    ...
    while ((line = br.readLine()) != null) {
      ...
      for (int i = 0; i < line.length(); i++) {
        ...
        // Keep only characters below or equal to the limit
        if (code <= limit) {
          sb.append(ch);
        }
      }
